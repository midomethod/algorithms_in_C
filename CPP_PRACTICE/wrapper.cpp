...
int main(int argc, char** argv)
{
    ...
    {
        throw std::invalid_argument("Usage: ./wrapper <INPUT FILE> <OUTPUT FILE>"); // throw error
    }
    ...
    char *com, *dummy, *valstr, *op, *arg; // for using with strtok, strtol
    int val; // the value from the command

    List myList; // initializing the linked list
    ...
    while(getline(input,command)) // get next line of input, store as repeat
    {
        ...
        arg = strtok(NULL, " \t"); // next token is value, as string (check out documentation on strtok, to understand the parsing)
