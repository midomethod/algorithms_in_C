...
{
    Node* cursor = find(N, a);;
    
    while(cursor != NULL)
    {
        Node* targ = find(cursor,b);  // See if b resides in d1 subtree
...
int BST :: width(Node* N, int level)
{
    if(N==NULL)
    {
        return 0;
    ...
    return width(N->right,level-1)+width(N->left,level-1);    
}
